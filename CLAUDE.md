...
- **bot.py** - Telegram bot entry point using python-telegram-bot. Handles commands and message handlers. Features:
  ...
  - Progressive status updates — separate messages for each processing stage (📸→🔍→💾→✅ for photos, ✍️→💾→✅ for text)
...
**Data Flow:** User sends photo/text → Progress messages sent at each stage (📸 received → 🔍 analyzing → 💾 saving → ✅ done) → Weight and food name parsed from caption/message → Gemini API analyzes with appropriate prompt → JSON response normalized → Food logged to Google Sheets → Emoji-rich nutrition breakdown returned → Calorie warning shown if target exceeded
